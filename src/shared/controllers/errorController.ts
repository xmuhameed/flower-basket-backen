...
const handleCastErrorDB = (err: ErrorResponse) => {
	const message = `Invalid ${(err as any).path}: ${(err as any).value}.`;
	return new AppError(message, 400);
};

const handleDuplicateFieldsDB = (err: ErrorResponse) => {
	const value = err.keyValue ? Object.values(err.keyValue)[0] : '';
	const message = `Duplicate field value: ${value}. Please use another value!`;
	return new AppError(message, 400);
};

const handleValidationErrorDB = (err: ErrorResponse) => {
...
const handlePrismaValidationError = (err: ErrorResponse) => {
	// Extract the error message from Prisma's validation error
	const message = err.message.split('\n').pop()?.trim() ?? 'Invalid input data';
	return new AppError(message, 400);
};

const errorController = (err: ErrorResponse, req: Request, res: Response, next: NextFunction) => {
